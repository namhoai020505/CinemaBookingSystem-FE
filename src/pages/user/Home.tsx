...
const getRealSlideIndex = (index: number) =>
  ((((index - FIRST_REAL_SLIDE_INDEX) % LAST_REAL_SLIDE_INDEX) + LAST_REAL_SLIDE_INDEX) %
...
export default function Home() {
  ...
  return (
    <div className="-m-4 bg-[#182437]">
      <section className="relative overflow-hidden bg-black" aria-label="Movie banners">
        <div className="relative h-[220px] sm:h-[330px] lg:h-[520px] xl:h-[620px]">
          ...
          >
            <FiChevronLeft className="h-9 w-9 sm:h-11 sm:w-11" aria-hidden="true" />
          </button>
          ...
          >
            <FiChevronRight className="h-9 w-9 sm:h-11 sm:w-11" aria-hidden="true" />
          </button>
            ...
            {mockHeroSlides.map((slide, index) => (
              <button
                ...
                className={`h-3 w-3 rounded-full border border-white/80 transition ${
                  activeSlideIndex === index ? 'bg-white' : 'bg-transparent hover:bg-white/50'
                }`}
              />
            ))}
          </div>
      ...
      <section className="bg-[#182437] px-4 pb-12 pt-8 sm:px-6 sm:pb-16 sm:pt-10">
        <div className="mx-auto max-w-6xl">
          <div className="mb-8 flex flex-wrap items-center justify-center gap-x-2 gap-y-1 text-center text-base font-extrabold uppercase text-white sm:text-xl">
            ...
            <button type="button" className="border-b-2 border-[#FFD166] pb-1 text-[#FFD166]">
              Phim đang chiếu
            </button>
            <span className="text-white/80">|</span>
            <button type="button" className="transition hover:text-[#FFD166]">
          ...
          <div className="grid grid-cols-1 gap-x-6 gap-y-10 sm:grid-cols-2 lg:grid-cols-4">
            {mockNowShowingMovies.map((movie) => (
              <article key={movie.id} className="flex min-w-0 flex-col">
                ...
                <p className="text-xs leading-5 text-white">
                  Thời lượng: <span className="text-white/80">{movie.duration}</span>
                </p>

                <button
                  type="button"
                  className="mt-5 h-10 w-full rounded-md bg-gradient-to-r from-[#FFD166] to-[#FFE7A3] text-xs font-extrabold uppercase text-black transition hover:brightness-105 focus:outline-none focus:ring-2 focus:ring-[#FFD166] focus:ring-offset-2 focus:ring-offset-[#182437]"
                >
                  Mua vé
                </button>
              </article>
